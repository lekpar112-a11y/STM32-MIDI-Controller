/*
 * SynthController.cpp - Versi 1.3.2
...
void sendMidiMessage(uint8_t* message, uint8_t size) {
    ...
    // 3. Kirim ke BLE via HM-10
    BLE_MIDI_SERIAL.write(message, size);
}
...
void setup() {
    ...
    Serial.println("Starting Multi-Interface MIDI Controller v1.3.2...");
    ...
    Serial.println("BLE HM-10 connected on Serial2.");

    #if defined(USBCON)
      Serial.println("USB MIDI Enabled.");
    #endif

    // (Tambahkan setup pin, encoder, atau matrix sesuai versi sebelumnya)
}

/* ---------- LOOP ---------- */
void loop() {
    // Loop utama (fungsi BLE.poll() dihapus karena HM-10 tidak perlu)
    // Ganti dengan pemanggilan fungsi utama kamu
    // contoh:
    // scanMatrix();
    // readAnalogs();
    // handleEncoder();
